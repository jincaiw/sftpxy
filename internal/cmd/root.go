...
func init() {
	...
	rootCmd.SetVersionTemplate(`{{printf "SFTPxy "}}{{printf "%s" .Version}}
`)
}
...
func addConfigFlags(cmd *cobra.Command) {
	...
	viper.BindEnv(configDirKey, "SFTPXY_CONFIG_DIR") //nolint:errcheck // err is not nil only if the key to bind is missing
...
file name is "SFTPxy" and therefore
"SFTPxy.json", "SFTPxy.yaml" and so on are
searched.
This flag can be set using SFTPXY_CONFIG_DIR
env var too.`)
	...
	viper.BindEnv(configFileKey, "SFTPXY_CONFIG_FILE") //nolint:errcheck
	cmd.Flags().StringVar(&configFile, configFileFlag, viper.GetString(configFileKey),
		`Path to SFTPxy configuration file.
...
This flag can be set using SFTPXY_CONFIG_FILE
env var too.`)
...
func addBaseLoadDataFlags(cmd *cobra.Command) {
	...
	viper.BindEnv(loadDataFromKey, "SFTPXY_LOADDATA_FROM") //nolint:errcheck
...
This flag can be set using SFTPXY_LOADDATA_FROM
env var too.
`)
	...
	viper.BindEnv(loadDataModeKey, "SFTPXY_LOADDATA_MODE") //nolint:errcheck
  ...
  1 - New users are added, existing users are
	  not modified
This flag can be set using SFTPXY_LOADDATA_MODE
env var too.
`)
	...
	viper.BindEnv(loadDataCleanKey, "SFTPXY_LOADDATA_CLEAN") //nolint:errcheck
...
can be set using SFTPXY_LOADDATA_CLEAN env var
too. (default "false")
`)
...
func addServeFlags(cmd *cobra.Command) {
	...
	viper.BindEnv(logFilePathKey, "SFTPXY_LOG_FILE_PATH") //nolint:errcheck
...
set using SFTPXY_LOG_FILE_PATH env var too.
`)
	...
	viper.BindEnv(logMaxSizeKey, "SFTPXY_LOG_MAX_SIZE") //nolint:errcheck
...
using SFTPXY_LOG_MAX_SIZE env var too. It is
...
`)
	...
	viper.BindEnv(logMaxBackupKey, "SFTPXY_LOG_MAX_BACKUPS") //nolint:errcheck
	cmd.Flags().IntVarP(&logMaxBackups, "log-max-backups", "b", viper.GetInt(logMaxBackupKey),
		`Maximum number of old log files to retain.
This flag can be set using SFTPXY_LOG_MAX_BACKUPS
...
empty.`)
	...
	viper.BindEnv(logMaxAgeKey, "SFTPXY_LOG_MAX_AGE") //nolint:errcheck
	cmd.Flags().IntVarP(&logMaxAge, "log-max-age", "a", viper.GetInt(logMaxAgeKey),
		`Maximum number of days to retain old log files.
This flag can be set using SFTPXY_LOG_MAX_AGE env
...
`)
	...
	viper.BindEnv(logCompressKey, "SFTPXY_LOG_COMPRESS") //nolint:errcheck
...
be set using SFTPXY_LOG_COMPRESS env var too.
...
`)
	...
	viper.BindEnv(logLevelKey, "SFTPXY_LOG_LEVEL") //nolint:errcheck
...
using SFTPXY_LOG_LEVEL env var too.
`)
...
`)
	...
	viper.BindEnv(loadDataQuotaScanKey, "SFTPXY_LOADDATA_QUOTA_SCAN") //nolint:errcheck
...
This flag can be set using SFTPXY_LOADDATA_QUOTA_SCAN
env var too.
(default 0)`)
	...
	viper.BindEnv(graceTimeKey, "SFTPXY_GRACE_TIME") //nolint:errcheck
...
interrupt signal.
This flag can be set using SFTPXY_GRACE_TIME env
