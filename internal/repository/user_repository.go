...
func (r *userRepository) GetByUsername(ctx context.Context, username string) (*User, error) {
	query := "SELECT id, username, status, password_hash, home_dir, filesystem, permissions, filters, quotas, bandwidth_limits, transfer_limits, max_sessions, allowed_protocols, denied_protocols, ip_filters, mfa_secret, mfa_enabled, expiration_date, description, created_at, updated_at FROM users WHERE username = ? LIMIT 1"

	var user User
	err := r.db.QueryRowContext(ctx, query, username).Scan(
		&user.ID, &user.Username, &user.Status, &user.PasswordHash,
		&user.HomeDir, &user.Filesystem, &user.Permissions, &user.Filters,
		&user.Quotas, &user.BandwidthLimits, &user.TransferLimits,
		&user.MaxSessions, &user.AllowedProtocols, &user.DeniedProtocols,
...
func (r *userRepository) GetByID(ctx context.Context, id int64) (*User, error) {
	query := "SELECT id, username, status, password_hash, home_dir, filesystem, permissions, filters, quotas, bandwidth_limits, transfer_limits, max_sessions, allowed_protocols, denied_protocols, ip_filters, mfa_secret, mfa_enabled, expiration_date, description, created_at, updated_at FROM users WHERE id = ? LIMIT 1"
...
func (r *userRepository) Create(ctx context.Context, user *User) (*User, error) {
	...
	result, err := r.db.ExecContext(ctx, query,
		user.Username, user.Status, user.PasswordHash, user.HomeDir,
		user.Filesystem, user.Permissions, user.Filters, user.Quotas,
		user.BandwidthLimits, user.TransferLimits, user.MaxSessions,
...
func (r *userRepository) Update(ctx context.Context, user *User) error {
	query := `UPDATE users SET
		status = ?, home_dir = ?, filesystem = ?, permissions = ?,
		filters = ?, quotas = ?, bandwidth_limits = ?, transfer_limits = ?,
		max_sessions = ?, allowed_protocols = ?, denied_protocols = ?,
		ip_filters = ?, mfa_secret = ?, mfa_enabled = ?,
		expiration_date = ?, description = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?`

	_, err := r.db.ExecContext(ctx, query,
		user.Status, user.HomeDir, user.Filesystem, user.Permissions,
...
func (r *userRepository) List(ctx context.Context, usernameFilter, statusFilter string, limit, offset int) ([]*User, error) {
	query := "SELECT id, username, status, password_hash, home_dir, filesystem, permissions, filters, quotas, bandwidth_limits, transfer_limits, max_sessions, allowed_protocols, denied_protocols, ip_filters, mfa_secret, mfa_enabled, expiration_date, description, created_at, updated_at FROM users WHERE 1=1"
	args := []interface{}{}
