...
// I18n* constants are localization keys for the Web frontend.
// They map 1:1 to dot-separated keys in static/locales/<lng>/translation.json
// (consumed client-side by i18next; the backend never renders translated text).
// The constants are grouped here by key prefix so each UI domain is easy to
// scan, extend and keep in parity with the translation catalogs. Only the
// `en` and `zh-CN` locales are shipped, but the i18next-based architecture
// supports adding more locales by dropping a new translation.json in place.
const (
	// --- Page titles (title.*) ---
	...
	// --- General errors and messages (general.*) ---
	...
	// --- Password change errors (change_pwd.*) ---
	...
	// --- Storage / filesystem config errors (storage.*) ---
	...
	// --- Virtual folder errors (virtual_folders.*) ---
	...
	// --- Event action errors and types (actions.*) ---
	...
	// --- Event rule errors and triggers (rules.*) ---
