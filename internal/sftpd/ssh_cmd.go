	...
	if c.command != scpCmdName {
		...
		if err == nil {
			logger.CommandLog(sshCommandLogSender, cmdPath, targetPath, vCmdPath, vTargetPath,
				c.connection.User.Username, "", c.connection.ID, common.ProtocolSSH, -1, -1, "", "",
				c.connection.command, -1, c.connection.GetLocalAddress(), c.connection.GetRemoteAddress(), elapsed)
		}
	}
}
