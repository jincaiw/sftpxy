...
import "github.com/jincaiw/sftpxy/sdk/kms"
...
type BaseS3FsConfig struct {
	...
	// Please note that if the upload bandwidth between the SFTP client and SFTPxy is greater than
	// the upload bandwidth between SFTPxy and S3 then the SFTP client have to wait for the upload
	// of the last parts to S3 after it ends the file upload to SFTPxy, and it may time out.
...
type BaseAzBlobFsConfig struct {
	...
	Endpoint string `json:"endpoint,omitempty"`
	// KeyPrefix is similar to a chroot directory for local filesystem.
	// If specified then the SFTPxy user will only see objects that starts
	// with this prefix and so you can restrict access to a specific
	// folder. The prefix, if not empty, must not start with "/" and must
	// end with "/".
	// If empty the whole bucket contents will be available
	...
	// Please note that if the upload bandwidth between the SFTPxy client and SFTPxy server is
	// greater than the upload bandwidth between SFTPxy and Azure then the SFTP client have
	// to wait for the upload of the last parts to Azure after it ends the file upload to SFTPxy,
	// and it may time out.
	// Keep this in mind if you customize these parameters.
	UploadPartSize int64 `json:"upload_part_size,omitempty"`
...
type BaseHTTPFsConfig struct {
	// HTTP/S endpoint URL. SFTPxy will use this URL as base, for example for the
	// "stat" API, SFTPxy will add "/stat/{name}"
	Endpoint string `json:"endpoint,omitempty"`
	Username string `json:"username,omitempty"`
